...
const GroupsCrud = () => {
  const { groupsId, stackId: stackPathId, teacherId: teacherPropId } = useParams()
  ...
  const [teacherId, setTeacherId] = useState<number | null>(teacherPropId ? Number(teacherPropId) : null)
  ...
  // Update
  const { mutate: GroupUpdate, isPending: updateLoading } = Update(cookies.token, `/groups/${groupsId}`, navigate, queryClient, QueryPATH.groupsMore, QueryPATH.groups)

  // Submit FN
  function handleSubmit(e: SubmitEvent<HTMLFormElement>) {
    e.preventDefault()
    const data = { name, stackId, teacherId, startDate, endDate, status }
    groupsId ? GroupUpdate(data) : GroupCreate(data)
  }

  // Get By Id for Update
  const { data: moreInfo = {} } = groupsId ? GetById(groupsId, cookies.token, QueryPATH.groupsMore, "/groups") : {}

  useEffect(() => {
    if (moreInfo && groupsId) {
    ...
    <form onSubmit={handleSubmit} className="p-5">
      <div className="flex items-center justify-between">
        <div className="flex items-center justify-center gap-2">
          ...
          <h1 className="font-bold text-[25px]">Gurux {groupsId ? "tahrirlash" : "qo'shish"}</h1>
        </div>
        <Button loading={groupsId ? updateLoading : createLoading} htmlType="submit" icon={<PlusCircleOutlined />} size="large" type="primary">Saqlash</Button>
      </div>
      <div className="flex items-center justify-center gap-5 m-5">
        <div className="w-[45%] flex flex-col gap-3">
          <label className="flex flex-col w-full! gap-1">
            ...
            <Input required value={name} onChange={(e) => setName(e.target.value)} className="w-full" allowClear size="large" placeholder="Gurux nomi" />
          </label>
          <label className="flex flex-col w-full! gap-1">
          ...
          <label className="flex flex-col w-full! gap-1">
            ...
            <CustomSelect extraClass="w-full!" disabled={teacherPropId ? true : false} URL="/teachers" placeholder="Ustoz tanlang" queryKey={QueryPATH.teachers} setValue={setTeacherId} value={teacherId} params={{ stackId }} />
          </label>
        </div>
        <div className="w-[45%] flex flex-col gap-3">
          <label className="flex flex-col w-full! gap-1">
